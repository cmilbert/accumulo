...
public abstract class AccumuloS3FileSystemBase extends FileSystem {
  ...
  public static final String USE_STATIC_CLIENT = "org.apache.accumulo.mss.file.useStaticClient";
  ...
  private AmazonS3 constructS3Client() {
    AmazonS3ClientBuilder builder =
        S3ClientBuilder.builder(getS3Credentials(), null)
  ...
  public FileStatus getFileStatus(Path path) throws IOException {
    ...
    if (meta != null) {
      ...
      return new FileStatus(meta.getContentLength(), false, 1, blockSize,
          lastModifiedDate.getTime(), 0, null, null, null, path);
    }
    ObjectListing listing = s3.listObjects(bucketName, getDirName(path));
    if (!listing.getObjectSummaries().isEmpty()) {
      // this is a directory
      return new FileStatus(0, true, 0, 0, 0, 0, null, null, null,
          new Path(getScheme() + "://" + bucketName + "/" + getObjectName(path)));
    }
  ...
  public boolean delete(Path path, boolean recursive) throws IOException {
    ...
    if (recursive) {
      String marker = null;
      ListObjectsRequest listReq =
          new ListObjectsRequest(bucketName, objectName + "/", marker, null, 1000);
      while (true) {
  ...
  public FileStatus[] listStatus(Path path) throws FileNotFoundException, IOException {
    ListObjectsRequest listObjectsRequest =
        new ListObjectsRequest(bucketName, getDirName(path), null, "/", 1024);
    ...
    do {
      ...
      for (S3ObjectSummary obj : listing.getObjectSummaries()) {
        ...
        statuses.add(new FileStatus(obj.getSize(), false, 1, blockSize, lastModifiedDate.getTime(),
            0, null, null, null, new Path(getScheme() + "://" + bucketName + "/" + obj.getKey())));
      }
      for (String prefix : listing.getCommonPrefixes()) {
        statuses.add(new FileStatus(0, true, 0, 0, 0, 0, null, null, null,
            new Path(getScheme() + "://" + bucketName + "/" + prefix)));
      }
  ...
  protected String getDirName(Path path) {
    ...
    if (objectName.isEmpty()) {
      // object keys don't start with "/", so we only put that in if we've specified something other
      // than the root
      return objectName;
    } else {
      return objectName + "/";
  ...
  private void closeMultipartUpload(String objectName, String uploadId) throws IOException {
    ...
    for (PartListing l = s3.listParts(req); l != null; l = l.isTruncated()
        ? s3.listParts(req.withPartNumberMarker(l.getNextPartNumberMarker())) : null) {
  ...
  public void closeMultipartFile(Path path, long retryTimeoutMillis, long giveUpTimeoutMillis)
      throws IOException {
    String objectName = getObjectName(path);
    ...
    while (true) {
      ...
      try {
        ...
        ListMultipartUploadsRequest baseRequest =
            new ListMultipartUploadsRequest(bucketName).withPrefix(objectName);
        ...
        while (true) {
          ...
          if (listing.isTruncated()) {
            listing = s3.listMultipartUploads(baseRequest.withKeyMarker(listing.getNextKeyMarker())
        ...
        if (e.getErrorType().equals(ErrorType.Service)) {
          // TODO: validate that this is a failure to close due to some parameter issue, such as the
          // original server uploaded another part since the
