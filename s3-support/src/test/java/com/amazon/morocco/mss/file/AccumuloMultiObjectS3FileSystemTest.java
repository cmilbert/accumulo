...
//import static com.amazon.morocco.test.MoroccoTestUtil.expectException;
...
public class AccumuloMultiObjectS3FileSystemTest extends MockS3TestBase {
    ...
    public void testFSOperations() throws IOException {
        ...
        try (FSDataInputStream in = fs.open(testFilePath)) {
            ...
            // validate contents
            assertArrayEquals("buffer read matches first buffer write", buffer, Arrays.copyOfRange(readBuffer, 0, buffer.length));
            assertArrayEquals("buffer read matches second buffer write", buffer2, Arrays.copyOfRange(readBuffer, buffer.length, readBuffer.length));
        }
    ...
    private AmazonS3 failTwiceWrapper(final AmazonS3 s3, final AtomicInteger counter) {
        ...
        when(amazonS3.listObjects(any(ListObjectsRequest.class))).then(AdditionalAnswers.delegatesTo(s3));
    ...
    public void testCreateWithNoClose() throws Exception {
        ...
        try (FSDataOutputStream created = fs.create(testFile)) {
            // File was created but not closed, which means we should be able to open the file and find no contents
