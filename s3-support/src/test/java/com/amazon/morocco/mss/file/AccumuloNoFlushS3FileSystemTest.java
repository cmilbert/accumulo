...
//            verify(amazonS3, times(1)).completeMultipartUpload(any(CompleteMultipartUploadRequest.class));
//        });
    }

    public void testReadWriteOperations(AmazonS3 amazonS3, int writeMBs, ThrowingRunnable validation) throws Exception {
        when(amazonS3.getObjectMetadata(anyString(), anyString())).then(AdditionalAnswers.delegatesTo(s3));
        ...
        when(amazonS3.initiateMultipartUpload(any(InitiateMultipartUploadRequest.class))).then(AdditionalAnswers.delegatesTo(s3));
        when(amazonS3.uploadPart(any(UploadPartRequest.class))).then(AdditionalAnswers.delegatesTo(s3));
        when(amazonS3.completeMultipartUpload(any(CompleteMultipartUploadRequest.class))).then(AdditionalAnswers.delegatesTo(s3));
        s3 = amazonS3;
