...
public class S3InputStream extends InputStream implements Seekable, PositionedReadable {
  ...
  public static class ExtraBufferAllocationTracker {
    ...
    private synchronized static boolean allocate(int size, Configuration conf) {
      if (maxExtraBufferToAllocate < 0) {
        maxExtraBufferToAllocate =
            conf.getLong(MAX_EXTRA_BUFFER_TO_ALLOCATE_KEY, MAX_EXTRA_BUFFER_TO_ALLOCATE_DEFAULT);
      }
  ...
  public S3InputStream(S3ClientWrapper s3, String bucketName, String objectName, Configuration conf)
      throws IOException {
    this.conf = conf;
  ...
  private void fillBufferFully(byte[] buffer, int inputOff, int inputLen, long objectPosition)
      throws IOException {
    int off = inputOff;
    int len = inputLen;
    long rangeEnd = objectPosition + len - 1;
    try (S3Object object = s3.getObject(
        new GetObjectRequest(bucketName, objectName).withRange(objectPosition, rangeEnd))) {
      if (object == null) {
        throw new IOException(String.format("object %s:%s (range %d:%d) not found", bucketName,
            objectName, objectPosition, rangeEnd));
      }
  ...
  private void fillBuffer(long start) throws IOException {
    if (start >= objectLength) {
      throw new IOException(
          "attempting to read past end of object: " + start + " >= " + objectLength);
    }
    ...
    // back up the start to fill a buffer in anticipation of future reads, but don't reread data in
    // the current buffer
    long offsetStart =
    ...
    if (length > currentBufferAllocation) {
      ...
      ExtraBufferAllocationTracker
          .deAllocate(currentBufferAllocation - Math.max(minBufferSize, length));
    }
