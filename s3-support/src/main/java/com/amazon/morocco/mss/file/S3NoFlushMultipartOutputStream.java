...
public class S3NoFlushMultipartOutputStream extends S3OutputStreamBase {
    ...
    public S3NoFlushMultipartOutputStream(S3ClientWrapper s3, String bucketName, String objectName, int bufferSize) {
    ...
    protected byte[] copyToFlushBuffer(byte[] circularBuffer, long startPos, long endPos) {
        ...
        if (circularStart < circularEnd || circularEnd == 0) {
            ...
            System.arraycopy(circularBuffer, 0, flushBuffer, firstCopyLength, flushBuffer.length - firstCopyLength);
        }
        return flushBuffer;
    }
